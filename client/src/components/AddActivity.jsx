...
function AddActivity () {
    ...
    const [errors, setErrors] = useState({name:"", details:"", season:"", });
    ...
    useEffect(() => {
     ...
     function handleSelect(id) {
         setInput({...input,countries: [...input.countries, id.target.value]
        })
    }

    function handleDelete(e) {
    setInput({...input,countries: input.countries.filter(c => c !== e)
        })}
...
<label>Activity: </label> 
  <input className={` ${style.select}`}type="text" value={input.name} name="name" onChange={(e)=> handleInputChange(e) } placeholder="Activity name..." />
    {errors.name && ( <p className={style.error}>{errors.name}</p>)}
                                

    <label className={` ${style.select2}`} >Season: </label>
 <input className={` ${style.select}`} type="text" name="season" onChange={(e)=> handleInputChange(e) } required>
 </input> 
 {errors.season && ( <p className={style.error}>{errors.season}</p>)}

{/* <label className={` ${style.select2}`} >Season: </label>
 <select className={` ${style.select}`}  name="season" onChange={(e)=> handleInputChange(e) } required>
...
{season.map(e => ( <option value={e} name="season" key={e} >{e}</option>))}
 ...
 <label>Difficulty: </label>
    <select  className={`${style.select}`}  name="difficulty" onChange={(e)=> handleInputChange(e) }required >
        <option value="" hidden>Choose an option</option>
        {difficulty.map(e => (  <option value={e} name="difficulty">{e}</option>))}
    </select>
            
<label>Duration: </label>
 <select className={`${style.select}`}  name="duration" onChange={(e)=> handleInputChange(e) } required>
    <option value="" hidden>Choose an option</option>
       {duration.map(e => (<option value={e} name="duration">{e}</option>))}
     </select>

 <label>Time: </label>
    <select className={` ${style.select}`} name="time" onChange={(e)=> handleInputChange(e) }>
        <option value="" hidden>Choose an option</option>
          {time.map(e => ( <option value={e} name="time">{e}</option>  ))}
              </select>                                           
                           
<label>Country: </label>
     <select className={`${style.select}`} onChange={(e)=> handleSelect(e) } required>
       <option className={style.select} value="" hidden>Select country</option>
          {countries.map(e => (<option value={e.id} name="countries" key={e.id} >{e.name}</option> ))}
         </select>
             <div>
               <ul>
                  <li  name="countries" className={` ${style.lista}`}>{input.countries.map(i =>
                     <div>{i} <button onClick={() => handleDelete(i)} type="button">❌</button>
                      </div>)}</li>
                          </ul>
                        </div>
 ...
 <label>Details: </label> 
<textarea  className={`${style.select}`} type="text" value={input.details} name="details" onChange={(e)=> handleInputChange(e) }placeholder="Detail activity..."></textarea>
{errors.details && ( <p className={style.error}>{errors.details}</p>)}

    {mostrarBoton && ( <button  className={`${style.button}`} type="submit">AddActivity</button>)}
...
//                                 <label>Time: </label>
//                                 <select className={` ${style.select}`}  onChange={handleSelectTime}>
//                                     <option value="" hidden>Choose an option</option>
//                                     {time.map(e => (
